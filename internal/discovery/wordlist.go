// Package discovery contains subdomain enumeration logic.
// This file defines the built-in wordlists used for DNS brute-force attacks.
package discovery

// DefaultWordlist is used for standard scans (~100 entries).
// These are common subdomain names based on:
// - Infrastructure (api, cdn, mail, ftp, vpn)
// - Environments (dev, staging, test, prod)
// - Services (admin, portal, dashboard, blog, shop)
// - Technical (git, db, redis, elastic, metrics)
var DefaultWordlist = []string{
	"www", "mail", "smtp", "pop", "imap", "ftp", "sftp",
	"api", "api2", "api-v2", "rest", "graphql", "grpc",
	"dev", "develop", "development", "staging", "stage", "stg",
	"test", "testing", "qa", "uat", "sandbox", "demo",
	"admin", "administrator", "panel", "dashboard", "manage", "management",
	"portal", "internal", "intranet", "corp", "corporate",
	"vpn", "remote", "rdp", "access",
	"cdn", "static", "assets", "media", "images", "files", "storage",
	"blog", "shop", "store", "cart", "checkout", "pay", "payment", "billing",
	"auth", "login", "sso", "oauth", "id", "identity", "account", "accounts",
	"support", "help", "helpdesk", "ticket", "status", "monitor", "health",
	"git", "gitlab", "github", "bitbucket", "svn", "code", "repo",
	"ci", "cd", "jenkins", "build", "deploy",
	"db", "database", "mysql", "postgres", "mongo", "redis", "elastic",
	"kibana", "grafana", "prometheus", "metrics", "logs", "logging",
	"docs", "doc", "documentation", "wiki",
	"s3", "bucket", "backup", "archive",
	"m", "mobile", "app", "apps", "ws", "socket",
	"v1", "v2", "v3", "beta", "alpha", "legacy", "old", "new",
	"mx", "mx1", "mx2", "ns", "ns1", "ns2",
	"cloud", "k8s", "kube", "docker", "vault", "consul",
	"crm", "erp", "jira", "confluence", "slack",
}

// DeepWordlist extends DefaultWordlist for --deep scans (~200 entries total).
// Adds more variations and less common subdomains. This increases scan time
// but may discover hidden infrastructure.
var DeepWordlist = append(DefaultWordlist, []string{
	"api3", "api4", "api-dev", "api-staging", "api-prod",
	"app1", "app2", "app3", "web", "web1", "web2",
	"backend", "frontend", "service", "services",
	"prod", "production", "live", "preview",
	"secure", "security", "vpn2", "vpn3",
	"data", "analytics", "bi", "reporting",
	"email", "exchange", "autodiscover", "webmail",
	"chat", "im", "video", "meet", "zoom",
	"upload", "download", "assets2", "img",
	"node", "node1", "node2", "proxy", "gateway",
	"firewall", "waf", "lb", "loadbalancer",
	"forum", "community", "events", "news",
	"careers", "jobs", "hr", "people",
	"legal", "privacy", "compliance", "audit",
	"feedback", "survey", "forms",
	"partner", "partners", "affiliate", "affiliates",
	"wholesale", "b2b", "enterprise",
	"training", "learn", "lms", "courses", "academy",
	"lab", "labs", "research", "r-and-d",
	"tools", "tool", "utils", "utilities",
	"report", "reports", "export",
	"webhook", "hooks", "notify", "notifications",
	"queue", "worker", "jobs2", "scheduler",
	"cache", "memcache", "session",
	"search", "solr", "opensearch",
	"registry", "hub", "mirror",
}...)
